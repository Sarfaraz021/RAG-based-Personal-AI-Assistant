...
const JobListingsContent = () => {
  ...
  React.useEffect(() => {
    const fetchJobsData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/auth/showjobs', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
...
const AlumniDirectoryContent = () => {
  ...
  React.useEffect(() => {
    const fetchProfilesData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/auth/profiles', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  return (
    ...
    //     <div key={profile._id} className="max-w-sm rounded overflow-hidden shadow-lg p-4 m-4">
    //       {/* Add a check for profilePicture existence and path correctness */}
    //       <img className="w-full" src={profile.profilePicture || 'path/to/default/image.jpg'} alt={`Profile ${index + 1}`} />
    //       <div className="px-6 py-4">
    //         <h3 className="text-xl font-semibold">  <span className="text-md"> Profile {index + 1}  -   </span> {profile.job}</h3>
    //         <p>Age: {profile.age}</p>
    //         <p>Job: {profile.job}</p>
    //       </div>
    //     </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-8 ">
      ...
      {profiles.map((profile, index) => (
        <div key={profile._id} className="bg-white p-4 rounded-md shadow-md flex flex-col">
          ...
          <div className="bg-blue-950 text-white p-4 rounded-md mb-4 ">
            <h3 className="text-xl font-bold">Job : {profile.job}</h3>
            {/* <p className="text-sm">Job : {profile.job}</p> */}
            <div className="flex justify-center items-center">
              <div>
                {/* <p className="text-sm text-white">Age: {alumni.age}</p> */}
                <p className="text-sm text-white">Age : {profile.age} </p>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            onClick={() => openChatbox(profile._id)}
          >
            Chat
          </button>
          ...
          {selectedAlumni === profile._id && <Chatbox profile={profile} onClose={closeChatbox} />}
        </div>
      ))}
    </div>
  );
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-300">
      ...
      <div className="w-full md:w-1/4 bg-white p-4 shadow-md">
        ...
        <ul>
          {/* ... (existing list items) */}
          <li
            className={`cursor-pointer mb-2 p-2 ${selectedFeature === "profile"
          ...
          <li
            className={`cursor-pointer mb-2 p-2 ${selectedFeature === "jobListings"
              ? "bg-blue-950 text-white rounded"
              : "hover:bg-gray-200"
              }`}
          ...
          <li
            className={`cursor-pointer mb-2 p-2 ${selectedFeature === "alumniDirectory"
              ? "bg-blue-950 text-white rounded"
              : "hover:bg-gray-200"
              }`}
